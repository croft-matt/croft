...
function getFileIcon(filename: string) {
  ...
  if (['pdf'].includes(ext)) return <FileText className="h-4 w-4 text-muted-foreground" />
    ...
    <div>
      ...
      <div className="flex items-center gap-3 rounded-lg border border-border bg-muted/50 px-3 py-2.5">
        ...
        <button
          ...
          className="shrink-0 flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-foreground hover:bg-muted transition-colors"
