  ...
  if (jobs.length === 0) {
    return (
      <div className="rounded-xl border border-border bg-card p-4">
        <p className="text-sm text-muted-foreground">No jobs in this room yet.</p>
      </div>
    )
  }
    ...
    <div className="rounded-xl border border-border bg-card p-4">
      ...
      {closedJobs.length > 0 && (
        <div className="mt-3 border-t border-border pt-3">
          ...
          {showClosed && (
            <div className="mt-2 divide-y divide-border opacity-50">
              {closedJobs.map((job) => (
                <div key={job.id} className="flex items-start gap-3 py-2.5 first:pt-0">
                  ...
                  <p className="text-sm text-muted-foreground line-through">{job.description}</p>
                </div>
              ))}
            </div>
          )}
