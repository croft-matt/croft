...
function SuggestionCard({
  ...
  return (
    <div className="rounded-xl border border-dashed border-border bg-card/50 px-4 py-3 opacity-80">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <p className="text-sm font-medium text-foreground">{suggestion.title}</p>
          {suggestion.preview && (
            <p className="text-xs text-muted-foreground mt-0.5">{suggestion.preview}</p>
          )}
        </div>
        <div className="flex items-center gap-1 shrink-0">
          <button
            ...
            disabled={isPending}
            className="flex items-center gap-1 rounded-md px-2.5 py-1.5 text-xs font-medium text-muted-foreground hover:bg-muted hover:text-foreground transition-colors disabled:opacity-50"
          >
