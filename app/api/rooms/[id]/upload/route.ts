...
export async function POST(
  ...
  // RLS verifies the room belongs to the caller's workspace.
  ...
  for (const file of files) {
    ...
    const storagePath = `${room.workspace_id}/${room.id}/${assetId}${ext ? `.${ext}` : ''}`
    ...
    const likelyType = deriveLikelyType(file.type ?? '', file.name)
    ...
    if (insertError) {
      // Clean up the uploaded blob if the DB insert fails.
